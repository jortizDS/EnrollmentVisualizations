    ...
    column(width = 3,
           wellPanel(
             ...
             selectizeInput('level', 
                            ...
                            choices = c("Show all degree levels" = "")),
...
server <- function(input, output, session) {
  ...
  observeEvent(c(input$year, input$semester), {
    # Ensure inputs are available
    req(input$year, input$semester)  
    
    colleges_df <- enrollment_data()  %>%
      filter(Degree == "College total")
    
    college_choices <- c("Show all colleges" = "", setNames(colleges_df$Coll, colleges_df$`Major Name`))
  ...
  ## Update degree program choices
  observeEvent(c(input$college), {
    # Ensure inputs are available
    req(input$college)  
    
    # Filter the data
    filtered_programs <- enrollment_data() %>%
      filter(Coll == input$college) %>%
      select(programtype) %>%
      na.omit() %>%
      distinct() %>% 
      arrange(programtype) %>% pull()
    
    # Extract majors safely as a vector
    prog_choices <- c("Show all degree levels" = "", as.vector(filtered_programs))
